...
namespace ServiceModelEx
{
   public class GenericResolver : DataContractResolver
   {
      ...
      {
         Assembly[] assemblyReferences = GetCustomReferencedAssemblies();

         List<Type> types = new List<Type>();
         
         foreach(Assembly assembly in assemblyReferences)
         {
         ...
         else
         {
            ...
            {
               foreach(Assembly assembly in AppDomain.CurrentDomain.GetAssemblies().Where(domainAssembly=>IsDomainAssembly(domainAssembly)).Where(assembly => assembly.FullName.StartsWith("Miros.Sim")))
               {
      ...
      static Assembly[] GetCustomReferencedAssemblies()
      {
         ...
         return assemblies.Where(assembly => assembly.FullName.StartsWith("Miros.Sim")).ToArray();   //??     
      }
      ...
      static Assembly[] GetCustomReferencedAssemblies(Assembly assembly)
      {
         ...
         List<Assembly> assemblies = new List<Assembly>();
         foreach(AssemblyName assemblyName in referencedAssemblies.Where(name=>IsSystemAssembly(name) == false).Where(refAssembly => refAssembly.FullName.StartsWith("Miros.Sim"))) //??
         {
            try
            {
            ...
            //catch (System.IO.FileLoadException) { }//loosescrew
            catch {throw;}
         }
      ...
      {
            //loosescrew
            //if (assembly.FullName.Contains("CSScriptLibrary") || assembly.FullName.Contains("eval"))
            //{
            //    return new Type[0];
            //}
