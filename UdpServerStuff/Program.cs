...
namespace UdpServerStuff
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IPAddress send_to_address = GetLocalAddress();
            #region comments
        ...
        private static IPAddress GetLocalAddress()
        {
            foreach (IPAddress ip in Dns.GetHostEntry(Dns.GetHostName()).AddressList)
            {
                ...
                {
                    return IPAddress.Parse(ip.ToString().Substring(0, ip.ToString().LastIndexOf(".")) + ".255");
                }
            }
            return IPAddress.Any; //this should never happen
        }
    }
}